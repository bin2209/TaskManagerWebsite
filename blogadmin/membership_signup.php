<?php
	...
	if($_POST['signUp'] != ''){
		...
		sql("INSERT INTO `membership_users` set memberID='$memberID', passMD5='".md5($password)."', email='$email', signupDate='".@date('Y-m-d')."', groupID='$groupID', isBanned='0', isApproved='".($needsApproval==1 ? '0' : '1')."', custom1='$custom1', custom2='$custom2', custom3='$custom3', custom4='$custom4', comments='form'", $eo);
	...
	$groupsDropDown = preg_replace('/<option.*?value="3".*?><\/option>/i', '', htmlSQLSelect('groupID', "select groupID, concat(name, if(needsApproval=1, ' *', ' ')) from membership_groups where allowSignup=1 order by name", ($cg == 1 ? sqlValue("select groupID from membership_groups where allowSignup=1 order by name limit 1") : 0 )));
					...
					<form method="post" action="membership_signup.php">
						...
						<!-- 	<label for="group" class="control-label">Group</label> -->
							<select class="form-control" name="groupID" id="groupID">
								<option value="3" selected="" class="">authors </option>
							</select>						
						</div>
						...
						<div class="row">
							<div class="col-sm-offset-3 col-sm-6">
								<button class="btn btn-primary btn-lg btn-block" value="signUp" id="submit" type="submit" name="signUp">Đăng ký</button>
							</div>
						</div>
